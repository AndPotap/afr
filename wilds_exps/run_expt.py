...
from wilds_algorithms.initializer import initialize_algorithm, infer_d_out
from transforms import initialize_transform
from wilds_models.initializer import initialize_model
from wilds_configs.utils import populate_defaults
import wilds_configs.supported as supported
...
def main():
    ...
    parser.add_argument('--civilcomments_remove_minority', default=False, type=parse_bool, const=True, nargs='?',
                        help='CivilComments, remove all comments without any identities from the data.')
    ####################################################################################################################
    ...
    for split in full_dataset.split_dict.keys():
        ...
        if split == 'train':
            ############################################################################################################
            # Drop DFR reweighting data
            from wilds.datasets.wilds_dataset import WILDSSubset
            import numpy as np

            if config.dfr_reweighting_drop:
                print(f'Dropping DFR reweighting data, seed {config.dfr_reweighting_seed}')

                idx = datasets[split]['dataset'].indices.copy()
                rng = np.random.default_rng(config.dfr_reweighting_seed)
                rng.shuffle(idx)
                n_train = int((1 - config.dfr_reweighting_frac) * len(idx))
                train_idx = idx[:n_train]
                
                print(f'Original dataset size: {datasets[split]["dataset"].metadata_array.shape[0]}')
                datasets[split]['dataset'] = WILDSSubset(
                    full_dataset,
                    indices=train_idx,
                    transform=transform)
                print(f'New dataset size: {datasets[split]["dataset"].metadata_array.shape[0]}')
            if config.civilcomments_remove_minority:
                print(f'Removing minority groups')

                identities_mentioned = (datasets[split]['dataset'].metadata_array[:, :8].sum(axis=1) > 0)
                toxic = datasets[split]['dataset'].metadata_array[:, -2] == 1
                mask = (identities_mentioned & toxic) | (~identities_mentioned & ~toxic)
                train_idx = datasets[split]['dataset'].indices.copy()[mask]
                print(f'Original dataset size: {datasets[split]["dataset"].metadata_array.shape[0]}')
                datasets[split]['dataset'] = WILDSSubset(
                    full_dataset,
                    indices=train_idx,
