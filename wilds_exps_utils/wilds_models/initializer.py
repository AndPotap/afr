...
from wilds_models.layers import Identity
from wilds_utils import load
...
def initialize_model(config, d_out, is_featurizer=False):
    ...
    if config.model in ('resnet18', 'resnet34', 'resnet50', 'resnet101', 'wideresnet50', 'densenet121'):
        ...
        from wilds_models.resnet_multispectral import ResNet18
        if featurize:
        ...
        from wilds_models.gnn import GINVirtual
        if featurize:
        ...
        from wilds_models.code_gpt import GPT2LMHeadLogit, GPT2FeaturizerLMHeadLogit
        ...
        from wilds_models.CNN_genome import UNet
        if featurize:
...
def initialize_bert_based_model(config, d_out, featurize=False):
    from wilds_models.bert.bert import BertClassifier, BertFeaturizer
    from wilds_models.bert.distilbert import DistilBertClassifier, DistilBertFeaturizer
...
def initialize_fasterrcnn_model(config, d_out):
    from wilds_models.detection.fasterrcnn import fasterrcnn_resnet50_fpn
